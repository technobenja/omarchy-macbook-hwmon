...
## v6 + v7 AS EXECUTED — 2026-09-25 (released 1.3.0, then 1.3.1)

**1.3.0** (master `ee459fa`, tag `v1.3.0`, GitHub release latest at 15:18 UTC): deployed 08:07 PDT, post-copy gate `staged=ok`, shell PID 502643 → 898166. A1 `stale:false` 0.8 s, control `hwmon —` → recovered +2 s; A2 schema 4, nas_backup `fresh` 36,584 s vs file 36,604 s (60 s cache), home `fresh`, upower `ok`; **A3 FOUND A REGRESSION:** the longer UPower value elided its label to "UPower vs ba…" → label "UPower check" (`6dfc8f2`), reinstalled, re-screenshot clean; A4 new PID + IPC, 0 warnings; **A5 (independent monitoring agent): CPU 0.88 % over 328.7 s (≤ 1.2 %), VmRSS 26.6 MB (≤ 40 MB)**, PID stable; A6 identical; A7 scrub clean on the tree and the notes. Final advisor go/no-go: **GO**. Logged, not fixed: the startup `raw_points_for_events` scan took 1.26 s over 74,945 rows (existing behaviour); the README has named iFixit since v1.0.
**1.3.1** (master `2ff02c1`, tag `v1.3.1`, GitHub release latest): the owner's layout feedback, as v7 above. Gate `staged=ok`, shell PID 902513 → 906740, IPC `stale:false` 0.2 s, package identical, 0 warnings, A7 clean. A3 screenshots: System BACKUPS = Home snapshots + NAS backup; Hardware BATTERY ends with a full-width "UPower check", not elided. **Logged for later:** on that same page "Charge 105 %" (capacity vs *design*) sits above "battery 100.0 %" (charge_now/charge_full) — both correct, possibly puzzling; consider labelling Charge "vs design".
